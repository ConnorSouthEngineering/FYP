...
export class CreateModelsComponent { 
  ...
  constructor(private cdr: ChangeDetectorRef,
    ...
    toggleClassSelection(classItem: string, event: Event): void {
      const inputElement = event.target as HTMLInputElement;
      if (inputElement !== null) { 
        const isChecked = inputElement.checked;
        if (isChecked) {
          this.chosenClasses.push(classItem);
        } else {
          const index = this.chosenClasses.indexOf(classItem);
          if (index > -1) {
            this.chosenClasses.splice(index, 1);
          }
        }
      }
